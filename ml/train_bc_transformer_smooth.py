...
Checkpoint keys (consumed by export_artifact.py):
  model, obs_mean, obs_std, act_mean, act_std, meta, seq_len, args
...
def main():
    ...
    ap.add_argument(
        ...
        help="Observation window length. Must match dataset window T. "
             "Default: read from dataset shape (and validated against CFG.SEQ_LEN).",
    )
    ...
    dataset_seq_len = int(ds_tr.obs.shape[1])

    # Resolve and validate seq_len
    if args.seq_len is None:
        args.seq_len = dataset_seq_len
        print(f"[train] seq_len inferred from dataset: {args.seq_len}")
    elif args.seq_len != dataset_seq_len:
        raise SystemExit(
            f"--seq_len={args.seq_len} does not match dataset window T={dataset_seq_len}. "
            "Re-build the dataset or omit --seq_len to use the dataset value."
        )
    if args.seq_len != CFG.SEQ_LEN:
        print(
            f"[train] WARNING: seq_len={args.seq_len} differs from CFG.SEQ_LEN={CFG.SEQ_LEN}. "
    ...
    for ep in range(1, args.epochs+1):
        ...
        if va_loss < best_val:
            ...
            ckpt = {
                ...
                "meta": ds_tr.meta,
                "seq_len": args.seq_len,   # required by export_artifact.py
                "args": vars(args),
            }
