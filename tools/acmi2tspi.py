...
def resample_track(pts, dt, trim_speed_mps=50.0):
    ...
    # pitch/roll from ACMI are accumulated angles (can be ±100000°), not physical attitude.
    # They are not needed for AFSIM position replay — zero them out to prevent jitter.
    ...
    roll_i  = np.zeros_like(grid)
    pitch_i = np.zeros_like(grid)
    ...
    # Some ACMI exports provide near-zero speed in T field even while moving.
    # Guard against frozen TSPI playback by enforcing a minimum from position derivative.
